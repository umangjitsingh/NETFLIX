...
function VideoTitle({title, overview}) {
   ...
   return (
      <div className={`${TMDB_Movies ? " px-3 pb-4 pt-6 rounded-md w-88 " : " absolute top-[290px]  left-[4%] z-10 py-2  scale-85  max-h-90 "}`}>
         <h1
            className={`${
               TMDB_Movies ? "text-[28px] font-normal" : "text-6xl font-bold"
            } [text-shadow:_12px_12px_18px_rgb(1_2_41_/_0.9)]  ${fontOptions[index]} text-red-100 tracking-tighter`}
         >
            {title}
         </h1>
         <p className={` ${TMDB_Movies ? "max-w-[80%] py-7  mb-4 break-words line-clamp-6" : " max-w-[48%] py-4  font-headline text-lg text-zinc-50 break-words"}`}>{overview}</p>
